...
# aqui vc pode usar a logica de acordo com a origem dos dados, ou pegar somente a ultima informacao ou fazer o merge / update 
# de todos os dados, vai depender de como funciona a atualização dos dados na origem
# para esse estudo isso nao foi considerado por que nao temos detalhes de como funcina a fonte de dados, 
# utilizamos um filtro para pegar somente a "ultima data de processamento" somente para manter integridade dos processo visto que os dados 
# na bronze irão se duplicar ao longo do tempo caso o retorno da API seja sempre o mesmo.
...
query = f"""Select 
            ...
            latitude,
            phone,
            upper(website_url) as website_url ,
            upper(state) as state,
            upper(street) as street,
            anomesdia
            from {source_database_name}.{source_table_name} where anomesdia=(select max(anomesdia) from  {source_database_name}.{source_table_name})"""
...
wr.athena.to_iceberg(
    ...
    temp_path='s3://373249867868-silver-layer/data/temp',
    keep_files =False,
    table_location='s3://373249867868-silver-layer/data/api/openbrewerydb/breweries/',
